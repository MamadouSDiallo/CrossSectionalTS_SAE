...
### This will load the library where are stored the packages
#.libPaths("C:/Program Files/R/Cont_Library")
...
### Remove all objects to start fresh
#rm(list=ls())
...
covar <- function(rho_cov,var1,var2,M,T) {
  ...
  for (d in 1:M) {
    ...
    vcov_e[(1+(d-1)*T):(d*T),(1+(d-1)*T):(d*T)] <- Gamma*((sqrt_var_vec)%*%t(sqrt_var_vec))
  }
  diag(vcov_e)<- var
  return(vcov_e)
}
...
Theta_hat <- function(sig2_nu, sig2_u, rho, T, M, beeta, X, vcov_e, Stationarity) {
  ...
  if (toupper(Stationarity)=="FALSE") {
    ...
    for (t in 2:T) nu[,t] <- rho*nu[,t-1] + rnorm(M,mean=0,sd=sqrt(sig2_nu))
        ...
        # arima.sim produces a series of variance 1 so it is necessary to multiply by sqrt(sig2_nu) to get the right variance
        for (m in 1:M) nu[m,] <- arima.sim(n=T, model=list(order=c(1,0,0),ar=rho,sd=sqrt(sig2_nu)))*sqrt(sig2_nu) 
      } else {
...
## Gamma_nu and Gamma_u - functions to compute gamma_nu, gamma_u, and their first derivates for the dynamic and Rao-Yu model
Gamma_nu <- function(rho,T,Stationarity="FALSE") {
  Gamma_nu <- Gamma_nu.rho <- matrix(0,nrow=T, ncol=T) 
  if (rho==0) return(list(Gamma_nu=Gamma_nu, Gamma_nu.rho=Gamma_nu.rho)) 
  if (toupper(Stationarity)=="FALSE"){
    ...
    for (j in 2:T) {
      for (i in 1:j-1) {
        ...
        Gamma_nu.rho[i,j] <- (rho^(j-i-1))*sum((j-i+2*(0:(i-1)))*rho^(2*(0:(i-1))))
...
Gamma_u_dyn <- function(rho, T) {
  ...
  if (rho==0) return(list(Gamma_u=Gamma_u, Gamma_u.rho=Gamma_u.rho)) 
  rho_power <- rho^(0:(T-1))
...
logLik <- function(par, Model, Est.Method="reml", y, X, vcov_e, M, T, Stationarity, randomwalk, toll=1e-122) {
  ...
  if (sig2_nu <= 0 || sig2_u <= 0) return(NA)  
  ...
  V.A          <- t(A)%*%V%*%A
  #V.A.inv.P    <- A%*%solve(V.A,tol=toll)%*%t(A)
  #llike        <- -0.5*(determinant(V.A)$modulus[1] + t(y)%*%solve(V.A,tol=toll)%*%res)   
...
grad.logLik <- function(par, Model, Est.Method="reml", y, X, vcov_e, M, T, Stationarity, randomwalk, toll=1e-122) {
  ...
  if (tolower(Est.Method)=="mle") {
    ...
    res          <- y - X %*% B_tilde
    V.inv.res    <- V.inv%*%res
    grad.llike[1] <- -0.5*sum(diag(V.inv%*%gamma_nu)) + 0.5 * t(V.inv.res) %*% gamma_nu  %*% V.inv.res
    grad.llike[2] <- -0.5*sum(diag(V.inv%*%gamma_u))  + 0.5 * t(V.inv.res) %*% gamma_u   %*% V.inv.res
    grad.llike[3] <- -0.5*sum(diag(V.inv%*%V.rho)) + 0.5 * t(V.inv.res) %*% V.rho %*% V.inv.res
  } else if (tolower(Est.Method)=="reml") { 
      P            <- V.inv - V.inv%*%X%*%solve(Xt.V.inv.X,tol=toll)%*%t(X)%*%V.inv            # (p. 101)
      grad.llike[1] <- -0.5* sum(diag(P %*% gamma_nu)) + 0.5 * t(P%*%y) %*% gamma_nu %*% (P%*%y)
      grad.llike[2] <- -0.5* sum(diag(P %*% gamma_u))  + 0.5 * t(P%*%y) %*% gamma_u  %*% (P%*%y)
...
## Information matrix 
info.matrix <- function(par, Model, Est.Method="reml", y, X, vcov_e, M, T, Stationarity, randomwalk, toll=1e-22) {
...
# reml.dyn - restricted maximum likelihood estimation of the dynamic model
# requires DynGamma.R


mse.est <- function(par, Model, y, X, vcov_e, M, T, Stationarity, randomwalk, Est.Method="mle", toll=1e-22) {
...
# begin MSE calculation
  ...
  for (d in 1:M){
    ...
    } else {d.bi <- matrix(0,nrow=T,ncol=2)} 
    d.bi[,1] <- t(t(mi)%*%gamma_nu[1:T,1:T]%*%Vi.inv - t(mi)%*%Gi%*%Vi.inv%*%gamma_nu[1:T,1:T]%*%Vi.inv)
    d.bi[,2] <- t(t(mi)%*%gamma_u[1:T,1:T]%*%Vi.inv  - t(mi)%*%Gi%*%Vi.inv%*%gamma_u[1:T,1:T]%*%Vi.inv)
    if (toupper(randomwalk)=="FALSE") {
      d.Gi_rho <- (sig2_nu*gamma_nu.rho[1:T,1:T]+sig2_u*gamma_nu.rho[1:T,1:T])
      d.bi[,3] <- t(t(mi)%*%d.Gi_rho%*%Vi.inv - t(mi)%*%Gi%*%Vi.inv%*%(sig2_nu*gamma_nu.rho[1:T,1:T]+sig2_u*gamma_nu.rho[1:T,1:T])%*%Vi.inv)
    }
  ...
  return(list(coef=B_Est,Est=Est,Est_fixed=Est_fixed,compositing.weights=wt,mse=mse,mse.decomp=mse.decomp))
}
